...
function App() {
  ...
  const searchByRecipe = () => {
    setIsLoading(true);
    fetch(`https://api.edamam.com/search?q=${selectedRecipe}&app_id=key=`)
      .then((response) => {
          ...
          {recipe.length <= 0 && (
           <div className="row flex mt-5 justify-center">
              <div class="bg-white image-width border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                ...
                <div class="p-5">
                  <a href="#">
                    <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">No Recipe By this Ingred.</h5>
                  </a>
                  <p class="mb-3 font-normal text-yellow-700 dark:text-gray-400">No Servings</p>
                  <p class="mb-3 font-normal text-green-700 dark:text-green-400">NA Kcal</p>
                </div>
              </div>
            </div>
         ) }
